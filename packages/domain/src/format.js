...
/* A duration, in the reader's language and digits.
 *
 * The units were English letters glued onto a number, which is the one kind of string a
 * translation pass cannot find: there is no `t()` call to collect, and "1h 30m" looks like a
 * value rather than a sentence. In a Persian app it is the only Latin text on the screen. */
export function fmtDur(ms) {
  const m = Math.floor(ms / 60000)
  return m >= 60
    ? t('{0}h {1}m', fmtInt(Math.floor(m / 60)), fmtInt(m % 60))
    : t('{0} min', fmtInt(m))
}
...
/* A plain integer in the reader's digits — a day number, a year, an hour count.
 *
 * Separate from `fmtNum` because of the grouping: a year through a grouping formatter is
 * "۱٬۴۰۵". It lives here rather than in `calendar.js` so that `fmtDur` above can use it without
 * the two files importing each other. */
export const fmtInt = n =>
  Number(n).toLocaleString(dateLocale(), { useGrouping: false, maximumFractionDigits: 0 })
...
/* Order is the order the swatches appear in. The first swatch is the brand's own emerald
 * rather than a system colour, and it leads because it is what the app ships as. Its key is
 * still "red" from the previous identity: the key is persisted per account and synced, so
 * renaming it would strand every account that has chosen it. It is never shown to anyone. */
export const ACCENTS = { red: '#1fa774', lime: '#30d158', sky: '#0a84ff', orange: '#ff9f0a', violet: '#bf5af2', pink: '#ff375f', teal: '#40c8e0', gold: '#ffd60a' }
